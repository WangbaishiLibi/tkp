...
$(function(){
    ...
    $(document).on("click", ".op_delete", function(){
        var eid = $(this).attr("rowid");
        layer.confirm('确认删除？', {icon: 3, title:'提示'}, function(index){
            $.getJSON("../table/entity/delete",{id:eid, tbname:"TestPaper"},function(data){
                if(data.res){
                    table.reloadAjax();
                    layer.msg("删除成功！");
                }
    ...
    var tbname = "TestPaper";
    table = new DataTable({
        tbname : tbname,
        tabledom : $("#tab_testpaper"),
        ajax : {
        ...
        columns:[
            {name : "编号", field : "id"},
            {name : "试卷名", field : "paperName"},
            {name : "时长", field : "duration"},
            {name : "单选题", field : "single_choice_cnt"},
            {name : "多选题", field : "multiple_choice_cnt"},
            {name : "判断题", field : "true_false_cnt"},
            {name : "简答题", field : "text_answer_cnt"},
